...
#define MEASUREMENT_INTERVAL 60                     // How many seconds do you want between each measurement from the CO2 sensor? The minimum is 10 seconds.
...
// Mysensors security
#define MY_ENCRYPTION_SIMPLE_PASSWD "14xlZSd3"     // Be aware, the length of the password has an effect on memory use.
...
#define MY_NODE_ID 8                                // Giving a node a manual ID can in rare cases fix connection issues.
...
#define CO2_RX_PIN 4                                // The RX (receive) pin for the CO2 sensor. geel   = RX = #6 = #define CO2_RX_PIN 5  -> #4. This should be connected to the TX (transmit) pin on the sensor module.
#define CO2_TX_PIN 3                                // The TX (transmit) pin for the CO2 sensor. oranje  = TX = #5 = #define CO2_TX_PIN 6  -> #3. This should be connected to the RX (receive) pin on the sensor module.
...
#define CO_CHILD_ID  5                              // The child ID of the sensor that will be presented to the controller.
#define CO2_CHILD_ID 6                              // The child ID of the sensor that will be presented to the controller.
#define CO_OPINION_CHILD_ID 7                       // The child ID of the human readable opinion about the carbon monoxide level.
#define CO2_OPINION_CHILD_ID 8                      // The child ID of the human readable opinion about the carbon dioxide level.
#define DATA_TRANSMISSION_CHILD_ID 9                // The child ID of the data transmission switch.
...
void presentation()  
{
  ...
  sendSketchInfo(F("Eva Room Climate Sensor"), F("1.0")); wait(RADIO_DELAY);
...
void setup() {
  ...
  Serial.println(F("Hello, I am the Eva Room Climate sensor"));
...
void loop()
{
  ...
  if( transmission_button_state == 0 ){             // If the button is being pressed
    ...
    wait(500); // Wait a while to allow the button to be release
#ifdef ALLOW_CONNECTING_TO_NETWORK
  ...
  //
  // MAIN LOOP (### see HEARTBEAT LOOP in HASS-IoT-6)
