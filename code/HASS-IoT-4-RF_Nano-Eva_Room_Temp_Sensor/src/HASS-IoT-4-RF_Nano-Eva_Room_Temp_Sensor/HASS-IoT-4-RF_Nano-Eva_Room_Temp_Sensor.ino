...
#define MY_NODE_ID 4                             // Giving a node a manual ID can in rare cases fix connection issues.
...
void presentation()  {
  ...
  sendSketchInfo(F("Eva Room Temp Sensor"), F("1.0")); wait(RADIO_DELAY);
...
void setup() {
  ...
  Serial.println(F("Hello, I am the Eva Room temperature and more sensor"));
  ...
  if( millis() - previousMillis >= INTERVAL ){      // Main loop, runs every second.
    ...
    switch (intervalCounter) {
      ...
      case 0:                                       // If we are in the first second of the clock
        ...
        temperature = bme280.getTemperature() - 1.7; // Adjust temperature by subtracting 1 degree for calibration purposes;
