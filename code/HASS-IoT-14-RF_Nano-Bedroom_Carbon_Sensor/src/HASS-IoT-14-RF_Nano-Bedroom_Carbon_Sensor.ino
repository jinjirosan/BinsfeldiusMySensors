...
#define MY_NODE_ID 14                                // Giving a node a manual ID can in rare cases fix connection issues.
...
#define CO2_RX_PIN 5                                // The RX (receive) pin for the CO2 sensor. geel   = RX = #6 = #define CO2_RX_PIN 5  -> #4. This should be connected to the TX (transmit) pin on the sensor module.
#define CO2_TX_PIN 6                                // The TX (transmit) pin for the CO2 sensor. oranje  = TX = #5 = #define CO2_TX_PIN 6  -> #3. This should be connected to the RX (receive) pin on the sensor module.
...
void presentation()
{
  ...
  sendSketchInfo(F("Eva Room Carbon Sensor"), F("1.0")); wait(RADIO_DELAY);
...
void setup() 
{
  ...
  Serial.println(F("Hello, I am the Bedroom Carbon Sensor"));
